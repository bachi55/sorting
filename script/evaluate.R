...
for (sce in scenarios) {
  ...
  algorithms <- c("insertionSort", "quicksort", "mergesort")
  ...
  algorithmsNames <- c("insertion sort", "quicksort", "mergesort")
  ...
  ggplot (df, aes (x=n, y=mean)) +
    ggtitle (sce) +	
    ylim(0, 1000) + xlab ("n") + ylab ("times (us)") +
  ...
  ggplot (df, aes (x=n, y=mean)) +
    ggtitle (sce) +
    ylim(0, 1600000) + xlab ("n") + ylab ("cylces (#)") +
    geom_line (aes (group=algorithm, color=algorithm)) +
    geom_ribbon (aes (ymin=min, ymax=max, color=algorithm, fill=algorithm), alpha=0.5) +
    scale_color_manual(values=cbPalette) + 
    scale_fill_manual(values=cbPalette)  
